...
class FramesDataset(Dataset):
    """
    Dataset of videos. Each video can be:
      - '.mp4' or '.gif'
      - folder with frames (not supported here)
    """
    ...
    def __init__(
        ...
        self.root_dir = os.path.join(root_dir, subdir) if subdir is not None else root_dir
        self.frame_shape = tuple(frame_shape)
        self.is_train = is_train

        if not os.path.exists(self.root_dir):
            raise FileNotFoundError(f"Provided directory {self.root_dir} does not exist.")
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            source = np.array(video_array[0], dtype="float32").transpose((2, 0, 1))
            driving = np.array(video_array[1], dtype="float32").transpose((2, 0, 1))
            return source, driving
        else:
            video = np.array(video_array, dtype="float32").transpose((3, 0, 1, 2))
            return video
...
if __name__ == "__main__":
    train_dataset = FramesDataset(root_dir='../../video_enhancement/GFPGAN/pure_talking_faces', subdir='train', is_train=True)
    test_dataset = FramesDataset(root_dir='../../video_enhancement/GFPGAN/pure_talking_faces', subdir='test', is_train=False)
